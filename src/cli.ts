 ...
 */
function findDuplicateSecretValue(
  secrets: SecretsMap,
  secretValue: string,
  excludeUuid?: string
): { uuid: string; name: string; placeholder: string } | null {
  for (const [uuid, data] of Object.entries(secrets)) {
    ...
    if (existingSecret === secretValue) {
      const name = typeof data === 'object' && data.name
        ? data.name
        : uuid;
      const placeholder = generatePlaceholder(uuid, data);
...
program
  ...
  .action(async (_options, command) => {
    try {
      ...
      console.error('Error listing index:', (err as Error).message);
      process.exit(1);
    }
  });
